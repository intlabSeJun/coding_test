...
def swap(sub_num, M):
    ...
    try:
        if M-sub_num == -1:
            return
        print('존재',list_N[M - sub_num:].index(list_N[M - sub_num-1] + 1))
        index = list_N[M - sub_num:].index(list_N[M - sub_num-1] + 1) + 2 - sub_num + 1
        print('index',index)
        buf = list_N[M - sub_num-1]
        list_N[M - sub_num-1] = list_N[index]
        list_N[index] = buf
...
list_N = list(range(1,N+1)) # 1~N까지 리스트로 저장. [,N]
list_N_str = list(map(str, list_N))

for _ in range(math.factorial(M)):
    print("".join(list_N_str[:M]))
    for k in range(N-M):
